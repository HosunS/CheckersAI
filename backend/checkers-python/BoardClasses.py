...
class Board:
    ...
    def show_board(self, fh=None):
        """
        Prints board to console or to file.
        @param fh: file object, in case we need to print to file
        @return :
        @raise :
        """
        print("   ", end="", file=fh, flush=True)
        print(*range(0, self.col), sep="  ", file=fh, flush=True)
        # index for debugging easily
        for i, row in enumerate(self.board):
            print(i, end="", file=fh, flush=True)
            # index for debugging easily
            for j, col in enumerate(row):
                king = self.board[i][j].is_king
                if king:
                    print("%3s" % str(self.board[i][j].get_color()).upper(), end="", file=fh, flush=True)
                else:
                    print("%3s" % str(self.board[i][j].get_color()).lower(), end="", file=fh, flush=True)
